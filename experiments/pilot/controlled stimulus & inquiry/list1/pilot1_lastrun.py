...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Swantje\\Desktop\\pilot_contr_inqu\\list1\\pilot1_lastrun.py',
...
introduction = visual.TextBox2(
     win, text='The goal of this study is to investigate how humans understand English utterances. The study consists of three parts. In each part, you will read the same  3-sentence statement by a speaker. Here is an example:', font='Open Sans',
     pos=(0, 0.35),     letterHeight=0.025,
...
intro_block2_2 = visual.TextBox2(
     win, text='Remember: In this part you will read a statements by a speaker and will indicate what you expect her to say  in the next sentence.\nClick "Continue" to start the experiment.', font='Open Sans',
...
introduction_5 = visual.TextBox2(
     win, text='Before you give any judgments, imagine the following scenario for the experiment: \n\nYou are at a party and have just got into the kitchen to get yourself a drink. While you search for a cup and something you would like to drink, you overhear a conversation by a couple of people behind you. They know each other well and meet regularly. You have seen them at other parties but never talked to them.', font='Open Sans',
...
read_context_2 = visual.TextBox2(
     win, text='One girl says:', font='Open Sans',
     pos=(0, 0.35),     letterHeight=0.03,
...
text2 = visual.TextStim(win=win, name='text2',
    ...
    languageStyle='LTR',
    depth=-2.0);
proposal = visual.TextStim(win=win, name='proposal',
    text='',
    font='Open Sans',
    ...
    languageStyle='LTR',
    depth=-3.0);
exp = visual.Slider(win=win, name='exp',
    startValue=None, size=(1.0, 0.03), pos=(0, -0.19), units=None,
...
intro_block2 = visual.TextBox2(
     win, text='Remember: In this part you will read the girl\'s statement and what she said next. You will evaluate whether the girl is certain about something.\nClick "Continue" to start part 2 of the study.', font='Open Sans',
     pos=(0, -0.15),     letterHeight=0.025,
     size=(1, 0.2), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
...
read_context = visual.TextBox2(
     win, text='Read again what the girl said and what she said next:', font='Open Sans',
     pos=(0, 0.35),     letterHeight=0.03,
     size=(1., 0.05), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
...
certainty_rate = visual.Slider(win=win, name='certainty_rate',
    startValue=None, size=(1.0, 0.03), pos=(0, -0.1), units=None,
...
intro_block2_3 = visual.TextBox2(
     win, text='Remember: In this part you will read the girl\'s statement again and will be asked to explain your judgment from part 1.\nClick "Continue" to start part 3 of the study.', font='Open Sans',
     pos=(0, -0.15),     letterHeight=0.025,
     size=(1, 0.2), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
...
heading = visual.TextStim(win=win, name='heading',
    ...
    languageStyle='LTR',
    depth=0.0);
prev_trial = visual.TextBox2(
     win, text='', font='Open Sans',
     pos=(0, 0.25),     letterHeight=0.025,
     size=(1, 0.15), borderWidth=4.0,
     color='white', colorSpace='rgb',
     opacity=None,
     bold=False, italic=False,
     lineSpacing=1.2,
...
Heading_2 = visual.TextStim(win=win, name='Heading_2',
    ...
    languageStyle='LTR',
    depth=-2.0);
explanation_enter = visual.TextBox2(
...
gotValidClick = False  # until a click is received
# keep track of which components have finished
instructions2Components = [logo1_3, text_instr_3, introduction_5, continue1_3, mouse_6]
...
while continueRoutine:
    ...
    # *logo1_3* updates
    if logo1_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        logo1_3.frameNStart = frameN  # exact frame index
        logo1_3.tStart = t  # local t and not account for scr refresh
        logo1_3.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(logo1_3, 'tStartRefresh')  # time at next scr refresh
    ...
    # *continue1_3* updates
    if continue1_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        continue1_3.frameNStart = frameN  # exact frame index
        continue1_3.tStart = t  # local t and not account for scr refresh
        continue1_3.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(continue1_3, 'tStartRefresh')  # time at next scr refresh
...
for thisTrials_block1 in trials_block1:
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    trial_expectednessComponents = [read_context_2, textbox, text2, proposal, exp, continue3, mouse_2]
    ...
    while continueRoutine:
        ...
        # *proposal* updates
        if proposal.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            proposal.frameNStart = frameN  # exact frame index
            proposal.tStart = t  # local t and not account for scr refresh
            proposal.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(proposal, 'tStartRefresh')  # time at next scr refresh
            proposal.setAutoDraw(True)
        
        # *exp* updates
        if exp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            exp.frameNStart = frameN  # exact frame index
            exp.tStart = t  # local t and not account for scr refresh
            exp.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(exp, 'tStartRefresh')  # time at next scr refresh
    ...
    for thisComponent in trial_expectednessComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_block1.addData('exp.response', exp.getRating())
...
gotValidClick = False  # until a click is received
# keep track of which components have finished
block3Components = [logo5_2, text, intro_block2_3, continue6_3, mouse_8]
...
# update component parameters for each repeat
prev_trial.reset()
prev_trial.setText("I'm so upset! We all know where Sue's boyfriend Tom is every Monday, only Sue doesn't. But lately she seems to be angry with Tom, that bastard.")
explanation_enter.reset()
continue2_2.reset()
# setup some python lists for storing info about the mouse_consent_2
mouse_consent_2.clicked_name = []
gotValidClick = False  # until a click is received
# keep track of which components have finished
explanationComponents = [heading, prev_trial, Heading_2, explanation_enter, continue2_2, mouse_consent_2]
...
while continueRoutine:
    ...
    # *prev_trial* updates
    if prev_trial.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        prev_trial.frameNStart = frameN  # exact frame index
        prev_trial.tStart = t  # local t and not account for scr refresh
        prev_trial.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(prev_trial, 'tStartRefresh')  # time at next scr refresh
    ...
    if explanation_enter.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        # keep track of start time/frame for later
        explanation_enter.frameNStart = frameN  # exact frame index
        explanation_enter.tStart = t  # local t and not account for scr refresh
        explanation_enter.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(explanation_enter, 'tStartRefresh')  # time at next scr refresh
...
for thisComponent in explanationComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('explanation_enter.text',explanation_enter.text)
# store data for thisExp (ExperimentHandler)
thisExp.nextEntry()
