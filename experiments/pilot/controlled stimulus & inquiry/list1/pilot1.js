...
let expName = 'pilot1';  // from the Builder filename that created this script
let expInfo = {
...
function trial_expectednessRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of trial_expectednessComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function trial_certaintyRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of trial_certaintyComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
